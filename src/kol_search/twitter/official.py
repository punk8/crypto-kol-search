...
class OfficialTwitterClient:
    ...
    name = "official"
    capabilities = XReadCapabilities(user_search=True, trends=True, post_lookup=True)
    ...
    def get_tweet(self, tweet_id: str) -> XTweet | None:
        ...
        for item in (payload.get("includes") or {}).get("users") or []:
            account = _parse_account(item)
            users_by_id[account.id] = account
        return _parse_post(data, users_by_id)
